    ...
    while i < len(lines):
        ...
        if re.match(r'^0x[0-9A-Fa-f]{6}$', line):
            ...
            while j < len(lines):
                ...
                # 处理键：ShortName, Description, Status (值在下一行)
...
class DtcCompareApp(QMainWindow):
    ...
    def load_file(self, file_path):
        ...
        try:
            if ext == '.xlsx':
                ...
                # 优先使用指定sheet
                if "Dianoseprotokoll" in wb.sheetnames:
                    ws = wb["Dianoseprotokoll"]
                else:
                    ws = wb.active
                    self.statusBar().showMessage(f"未找到 'Dianoseprotokoll'，使用默认工作表: {ws.title}")
                # 读取第一列所有非空单元格
                lines = []
        ...
        try:
            ...
            if not data['main'] and not data['perf']:
                ...
                QMessageBox.warning(self, "解析警告",
                    ...
                    f"请确认内容格式是否与示例一致（包含IPN_MAIN/IPN_PERF和阶段标题）。\n"
                    f"内容预览（前300字符）：\n{preview}")
                return
    ...
    def update_tables(self):
        self.update_overview_table(self.overview_main_table, 'main', resize=False)
        self.update_overview_table(self.overview_perf_table, 'perf', resize=False)
        self.unify_column_widths()

    def update_overview_table(self, table_widget, module, resize=True):
        table_widget.clear()
        if not self.files:
