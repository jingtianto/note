...
class FixedColumnTableWidget(QWidget):
    """并排两个表格，左侧固定列，右侧滚动列，垂直滚动同步，表头固定"""
    def __init__(self, fixed_cols=3, parent=None):
    ...
    def initUI(self):
        ...
        # 左侧固定列宽自适应，但禁止水平滚动
        self.left_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeToContents)
        self.right_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeToContents)
    ...
    def setColumnCount(self, cols):
        # 总列数，前fixed_cols在左表，其余在右表
        left_cols = min(self.fixed_cols, cols)
        right_cols = max(0, cols - self.fixed_cols)
        self.left_table.setColumnCount(left_cols)
        self.right_table.setColumnCount(right_cols)
    
    def setHorizontalHeaderLabels(self, labels):
        # labels 是完整表头列表
        left_labels = labels[:self.fixed_cols]
        right_labels = labels[self.fixed_cols:]
        self.left_table.setHorizontalHeaderLabels(left_labels)
        self.right_table.setHorizontalHeaderLabels(right_labels)
        # 开启表头换行
        for table in [self.left_table, self.right_table]:
    ...
    def setItem(self, row, col, item):
        if col < self.fixed_cols:
            self.left_table.setItem(row, col, item)
        else:
            self.right_table.setItem(row, col - self.fixed_cols, item)
    
    def item(self, row, col):
        if col < self.fixed_cols:
            return self.left_table.item(row, col)
        else:
            return self.right_table.item(row, col - self.fixed_cols)
    ...
    def setColumnWidth(self, col, width):
        if col < self.fixed_cols:
            ...
            self.right_table.setColumnWidth(col - self.fixed_cols, width)
    
    def columnWidth(self, col):
        if col < self.fixed_cols:
            return self.left_table.columnWidth(col)
        else:
            return self.right_table.columnWidth(col - self.fixed_cols)
...
class DtcCompareApp(QMainWindow):
    ...
    def setup_overview_tab(self):
        ...
        layout.setSpacing(4)
        self.overview_main_table = FixedColumnTableWidget(fixed_cols=3)
        self.overview_perf_table = FixedColumnTableWidget(fixed_cols=3)
    ...
    def update_overview_table(self, table_widget, module):
        ...
        table_widget.setRowCount(data_row_count)
        
        # ---- 设置表头 ----
        # 表头为单行，但文本使用换行符组合文件名和阶段名
        headers = ['DTC编号', '短名称', '描述']
        for f in self.files:
            # 文件名 + 阶段名，用换行分隔
            headers.append(f"{f.name}\nPreprocess")
            headers.append(f"{f.name}\nBefore")
            headers.append(f"{f.name}\nPost")
        table_widget.setHorizontalHeaderLabels(headers)
        
        # 启用表头换行（通过样式或设置项）
        for table in [table_widget.left_table, table_widget.right_table]:
            header = table.horizontalHeader()
            # 设置表头项自动换行（需要将表头项设置为可换行）
            for i in range(header.count()):
                item = header.model().headerData(i, Qt.Horizontal, Qt.DisplayRole)
                if item:
                    # 通过设置表头项的文本格式
                    header_item = header.sectionItem(i)
                    if header_item:
    ...
    def get_visible_table_data(self, table_widget):
        ...
        for col in range(table_widget.columnCount()):
            # 从左侧或右侧表头获取
            if col < table_widget.fixed_cols:
                header_item = table_widget.left_table.horizontalHeaderItem(col)
            else:
                header_item = table_widget.right_table.horizontalHeaderItem(col - table_widget.fixed_cols)
