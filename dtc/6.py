...
class FrozenTableWidget(QWidget):
    ...
    def setHorizontalHeaderLabels(self, labels):
        left_labels = labels[:self.freeze_cols]
        right_labels = labels[self.freeze_cols:]
...
class DtcCompareApp(QMainWindow):
    ...
    def update_overview_table(self, table_widget, module):
        ...
        for f in self.files:
            headers.append(f"{f.name} Preprocess")
    ...
    def get_visible_table_data(self, table_widget):
        if table_widget.rowCount() == 0:
            return None
        # 判断是否为 FrozenTableWidget
        if hasattr(table_widget, 'left_table'):
            left_headers = [table_widget.left_table.horizontalHeaderItem(i).text() if table_widget.left_table.horizontalHeaderItem(i) else f"Col{i}" for i in range(table_widget.left_table.columnCount())]
            right_headers = [table_widget.right_table.horizontalHeaderItem(i).text() if table_widget.right_table.horizontalHeaderItem(i) else f"Col{i+table_widget.left_table.columnCount()}" for i in range(table_widget.right_table.columnCount())]
            headers = left_headers + right_headers
        else:
            headers = [table_widget.horizontalHeaderItem(c).text() if table_widget.horizontalHeaderItem(c) else f"Col{c}" for c in range(table_widget.columnCount())]
        data = []
        for row in range(table_widget.rowCount()):
