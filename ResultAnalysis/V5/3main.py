...
# 依赖安装：pip install pyqt5 PyQtWebEngine flask flask-cors openpyxl -i https://pypi.tuna.tsinghua.edu.cn/simple
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .stat-card .stat-title { font-size: 15px; font-weight: 600; color: #1e293b; margin-bottom: 6px; padding-bottom: 4px; border-bottom: 1px solid #cbd5e1; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px; }
...
function getColorCss(colorName) {
    const map = {
        ...
        '红色': '#FF0000', '浅蓝色': '#99CCFF', 'blank': '#f1f5f9', '其他': '#e2e8f0'
    };
...
// ================================================================
// 文件上传（修复：静默跳过非excel文件，加密文件由后端跳过）
// ================================================================
function handleFiles(filesOrEvent) {
    ...
    for (let f of files) {
        // 修复1：静默跳过文件夹本身（没有扩展名的条目）和非 .xlsx/.xls 文件
        if (!f.name.endsWith('.xlsx') && !f.name.endsWith('.xls')) {
            // 如果文件没有扩展名，可能是文件夹，静默跳过
            if (!f.name.includes('.')) {
                continue;
            }
            // 其他非excel文件，静默跳过（不弹窗）
...
function buildMergedColumnTableNoAlign(fileResults, tbody, firstKw, defaultFontSize, defaultColumnWidth, bgColors, fileNames) {
    let maxRows = 0;
...
function buildMergedColumnTableMulti(fileResults, hasAlign, alignKeyIdx, scheme) {
    ...
    if (!hasAlign || alignKeyIdx === null || alignKeyIdx === undefined || alignKeyIdx === '') {
        ...
        buildMergedColumnTableNoAlign(fileResults, tbody, firstKw, defaultFontSize, defaultColumnWidth, bgColors, fileNames);
        table.appendChild(tbody);
        return table;
    }
    ...
    if (colorKw) {
        colorKwIndex = scheme.keywords.indexOf(colorKw);
    }
        ...
        displayItems.forEach((item, idx) => {
            ...
            if (!expanded && idx === 0) {
                ...
                fileNames.forEach(f => {
                    ...
                    // 修复6：使用 getDotClass 支持中英文
                    const dotClass = c ? getDotClass(c) : 'dot-blank';
...
async function exportIndependentExcel() {
    ...
    const alignKeyIdx = scheme.alignBaseKey;
    let processedResults = statisticsResults.results;
    if (alignKeyIdx !== null && alignKeyIdx !== undefined && alignKeyIdx !== '') {
        processedResults = alignColumnData(statisticsResults.results, alignKeyIdx);
    }

    for (let idx = 0; idx < processedResults.length; idx++) {
        const res = processedResults[idx];
        const displayName = res.folder ? `${res.folder}/${res.originalName || res.name}` : (res.name || res.originalName || '文件');

        let sheetName = displayName.substring(0, 28) + '_' + (idx + 1);
        ...
        if (cellData.length > 0) {
            ws.addRow(['单元格采集项']);
            const headers = ['关键字', '值'];
            const hasColor = cellData.some(d => d.color_indexed !== null && d.color_indexed !== undefined);
        ...
        const rowsToExport = (res.alignedRows || columnData.rows || []);
        if (rowsToExport.length > 0) {
            ...
            rowsToExport.forEach(row => {
                ...
                row.forEach((cell, idx) => {
                    ...
                    if ((columnData.keywords || [])[idx] && (columnData.keywords || [])[idx].addColorCol) {
                        const meaning = cell.color_meaning || 'blank';
                        values.push(meaning);
                    }
                });
...
async function exportMergedExcel() {
    ...
    const validFiles = processedResults.filter(r => r.data && r.data.hasColumn);
    ...
    let firstKw = null;
    for (let res of validFiles) {
        if (res.data.columnData && res.data.columnData.keywords && res.data.columnData.keywords.length > 0) {
            firstKw = res.data.columnData.keywords;
            break;
        }
    }
    ...
    if (hasAlign && alignValues && alignValues.length > 0) {
        const alignKeyIndex = parseInt(alignKeyIdx);
        if (!isNaN(alignKeyIndex) && alignKeyIndex >= 0) {
            const fileDataMap = {};
            validFiles.forEach((res) => {
                const rows = hasAlign ? res.alignedRows : res.data.columnData.rows;
                const filename = res.folder ? `${res.folder}/${res.originalName || res.name}` : (res.name || res.originalName || '文件');
                fileDataMap[filename] = rows || [];
            });

            const allFiles = validFiles.map(res => res.folder ? `${res.folder}/${res.originalName || res.name}` : (res.name || res.originalName || '文件'));

            alignValues.forEach((alignVal, rowIdx) => {
                allFiles.forEach((filename, fi) => {
                    const rows = fileDataMap[filename] || [];
                    const row = rows[rowIdx] || [];
                    if (row.length === 0) return;

                    const values = [filename];
                    firstKw.forEach((kw, idx) => {
                        const cell = (idx < row.length) ? row[idx] : { value: null, color_name: null, color_meaning: null };
                        values.push(cell.value !== null && cell.value !== undefined ? cell.value : '');
                        if (kw.addColorCol) {
                            const meaning = cell.color_meaning || 'blank';
                            values.push(meaning);
                    ...
                    rowObj.eachCell((cell) => {
                        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + bgColors[fi % bgColors.length] } };
                    });

                    firstKw.forEach((kw, idx) => {
                        const cell = (idx < row.length) ? row[idx] : null;
                        if (kw.collectColor && cell && cell.color_name && cell.color_name !== 'blank') {
                            const cellExcel = rowObj.getCell(idx + 2);
                            const argb = getColorArgb(cell.color_name);
                            if (argb) cellExcel.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: argb } };
                        }
                    });
            ...
            exportMergedNoAlign(ws, validFiles, firstKw, colorColIndices, bgColors);
        }
    } else {
        exportMergedNoAlign(ws, validFiles, firstKw, colorColIndices, bgColors);
    }
