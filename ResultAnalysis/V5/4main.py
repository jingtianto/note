...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .color-blank { background: #ffffff; color: #64748b; }  /* 修改为白色 */
...
const COLOR_MAP = {
    ...
    9:  { name: 'blank', meaning: 'blank', css: '#ffffff' }  // blank 为白色
};
const COLOR_MEANING = {
...
function syncKeywordsFromDOM() {
    ...
    rows.forEach(row => {
        ...
        const bold = row.querySelector('.kw-bold').checked;  // 新增
        if (sheet && text) {
            keywords.push({
...
function buildMergedColumnTableMulti(fileResults, hasAlign, alignKeyIdx, scheme) {
    ...
    groupKeys.forEach((key) => {
        ...
        items.forEach(item => {
            ...
            // 如果所有单元格都是 blank，rowPriority 仍为 999，设为最大优先级（最低）
        ...
        displayItems.forEach((item, idx) => {
            ...
            if (!expanded && idx === 0) {
                const fileColorMap = {};
                // 按照文件加载顺序（fileResults 顺序）填充颜色
                fileResults.forEach((res, fi) => {
                    const filename = res.folder ? `${res.folder}/${res.originalName || res.name}` : (res.name || res.originalName || '文件');
                    // 找到该文件在当前组中的行数据
                    const foundItem = items.find(it => it.filename === filename);
                    let color = 'blank';
                    if (foundItem) {
                ...
                // 调试日志
                log(`组 "${key}" 收起，颜色: ${fileNames.map((f,i) => f + ':' + (fileColorMap[f]||'blank')).join(', ')}`, 'info');
            } else {
