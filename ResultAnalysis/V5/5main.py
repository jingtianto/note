...
function getPriority(color, priorityOrder) {
    ...
    // 若 color 是英文但 priorityOrder 中是中文？但 priorityOrder 存的是英文，所以上面已处理
    // 若还找不到，返回 999
    return 999;
}
...
function buildMergedColumnTableMulti(fileResults, hasAlign, alignKeyIdx, scheme) {
    ...
    groupKeys.forEach((key) => {
        ...
        items.forEach(item => {
            ...
            if (rowPriority === 999) rowPriority = 999; // blank 最低
        ...
        // 构建文件名列表（按文件加载顺序）
        const orderedFileNames = fileResults.map(r => r.folder ? `${r.folder}/${r.originalName || r.name}` : (r.name || r.originalName || '文件'));
        ...
        displayItems.forEach((item, idx) => {
            ...
            if (idx === 0 && !isBest) tdFile.style.fontWeight = '500'; // 第一行但不是最优，保留一定区分度
            tr.appendChild(tdFile);
