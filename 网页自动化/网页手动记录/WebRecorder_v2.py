...
context = None
# 录制时希望浏览器"干净"：去掉那条不受支持标志的安全警告条、关掉翻译提示、首启弹窗
_launch_args = ["--start-maximized", "--disable-features=Translate,TranslateUI",
                "--no-first-run", "--no-default-browser-check",
                # 上次没正常关闭（比如被杀）时，Edge 会弹"还原页面/意外关闭"气泡，挡着页面
                "--hide-crash-restore-bubble"] + _extra_args

context = None
_last_err = None
for _attempt in ({"channel": "msedge", "chromium_sandbox": True},   # 首选：msedge + 沙箱（不再有 --no-sandbox 警告条）
                 {"channel": "msedge"},                            # 沙箱在本机不可用时退回
                 {"chromium_sandbox": True},                       # 没装 Edge 时用自带 chromium
                 {}):
    _label = _attempt.get("channel", "chromium（Playwright 自带）")
    _sandbox = "chromium_sandbox" in _attempt
    try:
        ...
        print(f"✅ 已启动浏览器：{_label}" + ("（沙箱开启）" if _sandbox else "（沙箱关闭）"))
        break
    except Exception as e:
        _last_err = e
        print(f"   启动尝试失败（{_label}{'/沙箱开启' if _sandbox else ''}）："
              f"{str(e).splitlines()[0][:110]}")

if context is None:
    print(f"❌ 启动浏览器失败：{_last_err}")
    print("   常见原因：Edge 已经开着（同一个 profile 目录不能被两个进程同时使用）")
    print(f"   → 请把所有 Edge 窗口关掉再重试（profile：{USER_DATA_DIR}）")
    try:
        pw_driver.stop()
    except Exception:
        pass
...
def maximize_window(pg):
    """把窗口真正最大化。
    --start-maximized 不可靠：持久化 profile 会记住上次的窗口尺寸并把它还原，
    所以这里直接用 CDP 改成 maximized。"""
    try:
        s = context.new_cdp_session(pg)
        wid = s.send("Browser.getWindowForTarget")["windowId"]
        s.send("Browser.setWindowBounds", {"windowId": wid,
                                          "bounds": {"windowState": "maximized"}})
        s.detach()
        return True
    except Exception as e:
        print(f"   （最大化失败，不影响录制：{str(e).splitlines()[0][:80]}）")
        return False

try:
    if maximize_window(page):
        print("🖥️  浏览器窗口已最大化")
except Exception as e:
    print(f"   （最大化异常，忽略：{e}）")
