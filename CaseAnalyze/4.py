...
class TestCaseParser:
    ...
    def parse_all(self):
        ...
        for row_idx, row, kategorie, signalname in all_cases_raw:
            ...
            all_signals = scene_signals + action_signals + expect_signals

            # 为每个信号赋类型（已在解析方法中设置，此处无需再设）
    ...
    def _parse_signalname(self, signalname_str):
        ...
        if '<<<' in text and '>>>' in text:
            ...
            if trigger_part:
                ...
                for sig in signals:
                    if sig.get('start') is None:
                        sig['start'] = times.get('start', '0')
                    if sig.get('end') is None:
                        sig['end'] = times.get('end', None)
        else:
            # 直接写法
    ...
    def _parse_expect_signals(self, row):
        ...
        for col_idx in range(self.randbedingungen_col, len(row) + 1):
            ...
            if text.startswith('==') or text.startswith('~=='):
                ...
                # 分离值和 t=...
                if ',t=' in rest:
                    value_part, time_part = rest.split(',t=', 1)
                    value_part = value_part.strip()
                    time_expr = time_part.strip()
                else:
                    value_part = rest
                    time_expr = None
                sig = {
